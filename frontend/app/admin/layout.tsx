...
import { LayoutDashboard, Users, Menu, X, FileText } from 'lucide-react';
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const getMenuClass = (path: string) => {
    ...
    const inactiveClass = "text-stone-500 hover:text-stone-800 hover:bg-[#FFFAF5]";
  ...
  return (
    <div className="flex min-h-screen bg-[#FFFAF5] font-sans">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white border-b border-stone-200 z-30 flex items-center justify-between px-5">
        ...
        <button onClick={toggleMenu} className="text-stone-600 p-1">
          <Menu size={24} />
        </button>
      </div>

      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-stone-900/50 backdrop-blur-sm z-40 md:hidden" onClick={closeMenu}></div>
      )}
      ...
      `}>
        <div className="p-6 h-16 md:h-[88px] flex items-center justify-between border-b border-stone-100">
          ...
          <button onClick={closeMenu} className="md:hidden text-stone-400 p-2">
            <X size={20} />
          </button>
        </div>
